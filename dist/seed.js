...
const destination_model_1 = require("./app/modules/Destination/destination.model");
const faq_model_1 = require("./app/modules/Faq/faq.model");
const testimonial_model_1 = require("./app/modules/Testimonial/testimonial.model");
const expert_model_1 = require("./app/modules/Expert/expert.model");
...
const seedData = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const s of services) {
            yield service_model_1.Service.findOneAndUpdate({ title: s.title }, s, { upsert: true, new: true });
        }
        const destinations = [
            {
                country: 'Australia',
        ...
        for (const d of destinations) {
            yield destination_model_1.Destination.findOneAndUpdate({ country: d.country }, d, { upsert: true, new: true });
        }
        const faqs = [
        ...
        for (const f of faqs) {
            yield faq_model_1.Faq.findOneAndUpdate({ question: f.question }, f, { upsert: true, new: true });
        }
        const testimonials = [
            {
                name: 'Clara Johnson',
        ...
        for (const t of testimonials) {
            yield testimonial_model_1.Testimonial.findOneAndUpdate({ name: t.name, role: t.role, text: t.text }, t, { upsert: true, new: true });
        }
        const experts = [
            {
                name: "Ahmed Al-Sayed",
        ...
        for (const e of experts) {
            yield expert_model_1.Expert.findOneAndUpdate({ name: e.name, category: e.category }, e, { upsert: true, new: true });
        }
